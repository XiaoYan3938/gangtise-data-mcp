...
私有录音、会议、云盘、股票池与企微消息等工具的远程 HTTP/SSE 入口。

> 本文假定服务**已经部署**。说明如何配置客户端连接本 HTTP/SSE 服务。  
> 业务实现见 [`mcp/gangtise_private`](../../mcp/gangtise_private/)。日常更推荐整合服务 [`gangtise_mcp`](../gangtise_mcp/)。
...
本包**没有** `--transport stdio`；本地 stdio 请用对应 [`mcp/gangtise_private`](../../mcp/gangtise_private/)。
