...
仅暴露 5 个域入口（list / read_ref / call 渐进披露），适合工具面需要收敛的场景。

> 本文假定服务**已经部署**。说明如何配置客户端连接本 HTTP/SSE 服务。  
> 日常更推荐整合服务 [`gangtise_mcp`](../gangtise_mcp/)。
...
`gangtise-agent`, `gangtise-data`, `gangtise-file`, `gangtise-kb`, `gangtise-private`（域入口，非叶子工具）。
...
本包**没有** `--transport stdio`；本地 stdio 请用对应 [`mcp/gangtise_hub`](../../mcp/gangtise_hub/)。
