...
五域全部叶子工具的远程 HTTP/SSE 入口。日常远程接入优先本服务。

> 本文假定服务**已经部署**。说明如何配置客户端连接本 HTTP/SSE 服务。  
> Docker 整合镜像默认即运行本服务（`MCP_LAYOUT=unified`）。
...
五域全部叶子工具（约 45 个），见 [`mcp/gangtise_mcp`](../../mcp/gangtise_mcp/)。
...
本包**没有** `--transport stdio`；本地 stdio 请用对应 [`mcp/gangtise_mcp`](../../mcp/gangtise_mcp/)。
