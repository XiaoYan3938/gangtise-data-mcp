...
本包为 **stdio** 业务入口；HTTP/SSE + 鉴权见 [`api/gangtise_file`](../../api/gangtise_file/)。在 monorepo 内：

```bash
cd gangtise-data-mcp/mcp/gangtise_file
uv sync
uv run gangtise-file-mcp          # stdio
# HTTP/SSE（api 包）
cd ../../api/gangtise_file && uv sync && uv run gangtise-file-api --transport both --host 0.0.0.0 --port 8000
```

CLI 调试见 [`cli/gangtise_file`](../../cli/gangtise_file/)。日常客户端接入推荐 [`gangtise_mcp`](../gangtise_mcp/)。
