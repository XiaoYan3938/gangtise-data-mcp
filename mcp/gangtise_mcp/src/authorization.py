"""Gangtise API 鉴权：单例 token 缓存，过期自动重登（常驻 MCP 进程自愈）。

stdio：使用环境变量 / 本地凭证文件。
http / sse：优先使用请求上下文中的 accessKey / secretKey（见请求头）。
"""
...
def authorized_request(method: str, url: str, *, retry_on_auth: bool = True, **kwargs) -> requests.Response:
    """带鉴权头发起请求；鉴权失败时强制刷新 token 并重试一次。"""
