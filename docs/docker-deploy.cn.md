...
仅提供 **整合镜像**（`mcps/Dockerfile`）：同一容器内装入 `api/*` + `mcp/*`，默认 gateway，客户端连 **`/mcp`**（全量叶子工具）。协议与鉴权见 [http-sse.md](http-sse.cn.md)。入口：[`mcp/gangtise_mcp/entrypoint.sh`](../mcp/gangtise_mcp/entrypoint.sh)。
