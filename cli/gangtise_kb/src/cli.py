...
def _check_auth_configured() -> Optional[str]:
    ...
    return (
        ...
        f"配置方式：gangtise-kb configure --access-key <AK> --secret-key <SK>"
...
def _build_epilog(spec_map: Dict[str, ToolSpec]) -> str:
    lines = ["子命令一览（gangtise-kb <命令> --help 查看参数）:", ""]
    listed: Set[str] = set()
    ...
    if other:
        ...
        lines.append("")
    lines.append("示例:")
    lines.append("  gangtise-kb configure --access-key <AK> --secret-key <SK>")
    lines.append("  gangtise-kb quote --securities 比亚迪")
    lines.append("  gangtise-kb security -k 比亚迪")
    lines.append("  gangtise-kb company-indicator -k 收盘价")
    lines.append("  gangtise-kb list")
    return "\n".join(lines)


def _cmd_list(spec_map: Dict[str, ToolSpec]) -> int:
    print(_build_epilog(spec_map).replace("子命令一览（gangtise-kb <命令> --help 查看参数）:", "可用命令:"))
    return 0
...
def _cmd_configure(args: argparse.Namespace) -> int:
    ...
    if args.show:
        if is_auth_configured():
            ...
            print(f"未配置授权。运行 gangtise-kb configure --access-key <AK> --secret-key <SK>")
        return 0
